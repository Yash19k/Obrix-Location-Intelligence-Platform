...
import { Bookmark, ArrowRight, Store, Coffee, ShoppingBag, BookOpen, MapPin } from 'lucide-react'
...
export default function SavedLocationsPreview({ savedLocations, isLoading }) {
  ...
  return (
    <div className="bg-white border border-[#DDE3EC] rounded-2xl p-5 shadow-2xs font-sans flex flex-col justify-between">
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[#E7F7E9] text-[#43B96B] flex items-center justify-center">
              <Bookmark className="w-4 h-4" />
            </div>
            <div>
              <h3 className="text-base font-extrabold text-[#08111F]">Saved Locations</h3>
              <p className="text-xs text-[#5D6675]">Bookmarked opportunities</p>
            </div>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-6 px-3 bg-[#F6F8FC] border border-[#DDE3EC] rounded-xl my-2">
            <p className="text-xs text-[#5D6675] font-medium">No saved locations yet.</p>
            <p className="text-[11px] text-[#8A94A3] mt-0.5">Bookmark promising sites during map analysis.</p>
          </div>
        ) : (
          <div className="space-y-2 my-2">
            {savedLocations.slice(0, 3).map((loc) => {
              const BizIcon = BIZ_ICONS[loc.business_type?.toLowerCase()] || MapPin
              const formattedBiz = loc.business_type
                ? loc.business_type.replace('_', ' ').replace(/\b\w/g, (c) => c.toUpperCase())
                : 'Saved Site'
              const latStr = loc.latitude ? parseFloat(loc.latitude).toFixed(4) : ''
              const lonStr = loc.longitude ? parseFloat(loc.longitude).toFixed(4) : ''
              const label = loc.title || (latStr ? `${latStr}° N, ${lonStr}° E` : 'Saved Location')
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {loc.score && (
                      <span className="text-xs font-mono font-bold text-[#43B96B] bg-[#E7F7E9] px-2 py-0.5 rounded-full border border-[#43B96B]/20">
                        {Math.round(parseFloat(loc.score))} / 100
                      </span>
                    )}
      ...
      >
        <span>View all saved locations</span>
