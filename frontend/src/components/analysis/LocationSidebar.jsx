  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
        ...
        {/* ── Saved Bookmarks (Priority 5) ─────────────────────────────────── */}
        <BookmarksSection />
      </div>

      {/* ── Action buttons — pinned to bottom ────────────────────────────── */}
      <div className="px-4 pb-4 pt-3 space-y-2 border-t border-white/[0.06] flex-shrink-0">

        {/* Compare Mode Button */}
        <button
          onClick={() => useMapStore.getState().startCompareMode()}
          className="w-full flex items-center justify-center gap-2
                     px-4 py-2 rounded-xl text-xs font-semibold uppercase tracking-wide
                     bg-purple-600/20 border border-purple-500/30 text-purple-300
                     hover:bg-purple-600/30 hover:text-white
                     transition-all duration-150"
        >
          <ArrowLeftRight className="w-3.5 h-3.5" /> Compare 2 Locations
        </button>
    ...
    <div className="space-y-2 pt-2 border-t border-white/[0.05]">
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-semibold text-white/40 uppercase tracking-widest flex items-center gap-1.5">
          <Bookmark className="w-3 h-3 text-indigo-400" /> Saved Locations
        </span>
        <span className="text-[10px] text-slate-500 font-mono">{savedLocations.length}</span>
      </div>
      ...
      <div className="space-y-1.5 max-h-44 overflow-y-auto pr-0.5">
        {savedLocations.map((loc) => (
          <div
            key={loc.id}
            className="group flex items-center justify-between p-2 rounded-lg bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.07] transition-all cursor-pointer"
            onClick={() => {
          ...
          >
            <div className="min-w-0 pr-2">
              <div className="text-xs font-medium text-slate-200 truncate">{loc.name}</div>
